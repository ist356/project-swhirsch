...
#a6p2 labels
players_to_label = ["Brock Purdy", "Tua Tagovailoa", "Josh Allen", "Bryce Young"]
label_offsets = [(0.5, -50), (1.5, 50), (-0.5, -25.0), (-2.0, 130)] 
for i, (index, row) in enumerate(data[data['Name'].isin(players_to_label)].iterrows()):
    offset = label_offsets[i % len(label_offsets)]
...
st.pyplot(plt)
st. caption('The relationship between passing attempts and touchdown percentage shows a weak positive correlation of 0.22. This suggests that as passing attempts increase, the touchdown percentage tends to rise slightly, but the connection is not strong. The linear model has a slope of 16.59 and an intercept of 179.01, reflecting this trend but not indicating a robust predictive relationship. On average, quarterbacks have a touchdown percentage of 3.15%. Those with more than 400 passing attempts perform better, averaging 4.24%, compared to 2.76% for quarterbacks with fewer attempts. This trend highlights how volume impacts scoring efficiency, with higher usage correlating to improved performance. Notable performers include Josh Allen and Tua Tagovailoa, both having a 5% touchdown rate on over 550 attempts. Brock Purdy stands out with an impressive 7% on 444 attempts. On the other hand, Panther’s rookie Bryce Young struggled, managing only a 2% touchdown rate on 527 attempts, underscoring the challenge of efficiency when having a high snap count.')